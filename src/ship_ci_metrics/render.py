...
_ROOT_IMPORT_ERROR = "PyROOT not found — install root/root_base in this environment"
...
def _require_root() -> Any:
    """Import PyROOT lazily on first use and configure batch mode and style."""
    global ROOT
    if ROOT is None:
        try:
            import ROOT as _ROOT
        except ImportError as exc:
            raise ImportError(_ROOT_IMPORT_ERROR) from exc
        _ROOT.gROOT.SetBatch(True)
        _ROOT.gStyle.SetOptStat(111111)
        _ROOT.gStyle.SetOptFit(1111)
        ROOT = _ROOT
    return ROOT
...
def render_object(obj: Any, output_path: Path) -> dict[str, str] | None:
    ...
    if not isinstance(obj, (ROOT.TH1, ROOT.TGraph, ROOT.TMultiGraph, ROOT.TEfficiency)):
        return None
